        ...
        for dose in insulin_doses:
            if dose['type'] == 'bolus':
                fig.add_vline(
                    ...
                    line_dash="dash",
                    line_color="blue",
                    annotation_text=f"💉 {dose['amount']}u",
...
def main():
    ...
    with st.sidebar:
        ...
        typical_basal = st.number_input("Typical Daily Basal (units)", min_value=0, max_value=100, value=st.session_state.typical_basal, key="typical_basal")
    ...
    if display_glucose:
        ...
        if st.session_state.meals and display_glucose:
            ...
            if recent_meals:
                ...
                with col3:
                    st.metric("Correction", f"{bolus_calc['correction_bolus']:.1f} units", f"IOB adjusted")
                with col4:
        ...
        with col2:
            if readings:
                ...
                if estimated_a1c:
                    st.metric("Estimated A1C", f"{estimated_a1c}%", f"Based on {len(readings)} readings")
            else:
    ...
    st.markdown("---")
    st.markdown("""
    **🧠 Bean's Bolus Brain Features:**
    - ✅ Real-time Dexcom integration with proper timezone handling
    - ✅ Insulin logging (bolus & basal) with IOB calculations
    - ✅ Smart correction suggestions (IOB-adjusted)
    - ✅ Apple Health integration ready
    - ✅ High/low identification with severity levels
    - ✅ 90-day estimated A1C calculation
    - ✅ Meal impact tracking with protein calculations
